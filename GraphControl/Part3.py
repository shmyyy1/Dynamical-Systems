...
T=10
step = int(T/0.1)
fig_fold = '/Users/shamanth/Desktop/Important/Penn/GNN Lab-5/lab5_code_v2/figures_part3/'
...
saveDirRoot = '/Users/shamanth/Desktop/Important/Penn/GNN Lab-5/lab5_code_v2/experiments'
...
print(f"DEBUG: saveDir (relative): {saveDir}")
print(f"DEBUG: saveDir (absolute): {os.path.abspath(saveDir)}")
...
print(f"Model will be saved to: {GraphNNs.saveDir}")
print(f"Starting training for {nEpochs} epochs with batch size {batchSize}")

batchsize = 5
# Train
GraphNNs.train(simulator, ref_vel, est_vel, est_vels, biased_vels, accels, com_network, nEpochs, batchSize)
print(f"Training complete! Check for saved models in: {os.path.join(GraphNNs.saveDir, 'savedModels')}")
